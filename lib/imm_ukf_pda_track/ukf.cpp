...
void UKF::initialize(const Eigen::VectorXd& z, const double timestamp, const int target_id)
{
  ...
  // 第一个sigma点的均值和协方差权重
  weights_s_(0) = weight_s_0; // 均值
  weights_c_(0) = weight_c_0; // 协方差
  ...
  // std_laspx_,std_laspy_:激光测量噪声标准偏差
...
void UKF::predictionSUKF(const double dt, const bool has_subscribed_vectormap)
{
  ...
  ****************************************************************************/
  // 预测CTRV运动模型: 生成sigma点，预测Sigma点的下一帧状态，预测系统状态的均值和方差
  predictionMotion(dt, MotionModel::CTRV);
  /*****************************************************************************
  *  Prediction Measurement
  ****************************************************************************/
  // 将状态空间预测出来的sigma点集 映射到测量空间然后算出均值和方差
 ...
 ****************************************************************************/
  // 恒速模型的 state vector: [pos1 pos2 vel_abs yaw_angle yaw_rate]
  ...
  ****************************************************************************/
  ...
  // Eigen的LLT分解实现了Cholesky 分解
  Eigen::MatrixXd L = p.llt().matrixL();

  // create augmented sigma points
  // 假定状态的个数为 n ，我们会产生 2n+1 个sigma点，其中第一个就是我们当前状态的均值 μ
  x_sig.col(0) = x;
  for (int i = 0; i < num_state_; i++)
  {
    // λ 越大， sigma点就越远离状态的均值，λ 越小， sigma点就越靠近状态的均值
  ...
  ****************************************************************************/
  // predict sigma points
  // 有sigma点集，根据process model g(χk,μk) 预测未来的Sigma点Xk+1∣k
  for (int i = 0; i < 2 * num_state_ + 1; i++)
  {
    // extract values for better readability
    double p_x = x_sig(0, i); // 第i列
  ...
  ****************************************************************************/
  // 根据预测的Sigma点Xk+1∣k ​生成状态预测xk+1∣k,Pk+1∣k
  // Xk+1∣k : sigma点集中每个点各个状态量的加权和
...
void UKF::predictionLidarMeasurement(const int motion_ind, const int num_meas_state)
{
  ...
  // 使用无损转换来解决,不用再产生sigma points了，直接使用预测出来的sigma点集
  for (int i = 0; i < 2 * num_state_ + 1; i++)
  {
  ...
  // z_sig：测量的sigma点 z_pred ：测量空间下预测均值 [2]
