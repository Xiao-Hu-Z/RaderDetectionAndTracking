...
bool ImmUkfPda::updateNecessaryTransform()
{
  ...
  try
  {
    // waitForTransform() 是用来等待某两个transform之间的连通
...
void ImmUkfPda::updateBehaviorState(const UKF& target, const bool use_sukf, autoware_msgs::DetectedObject& object)
{
  ...
  // mode_prob_cv_ = 0.33;  mode_prob_ctrv_ = 0.33; mode_prob_rm_ = 0.33;
...
// 初始id,位置，时间戳，协方差矩阵R,跟踪数量tracking_num
// 以输入的DetectedObjectArray和时间戳 初始化UKF的第一帧位置
                                             ...
                                             std::vector<autoware_msgs::DetectedObject>& object_vec, UKF& target)
{
  ...
  else
  {
    ...
    // 选取各种模型中行列式值 S 最大的 max_det_z，max_det_s
                            ...
                            const std::vector<size_t> in_tracker_indices)
{
  ...
  {
    // 将目标的position 存储入 池子centroids，保证每个中心位置不同
  ...
  for(size_t i=0; i< matching_objects.size(); i++)
  {
    ...
    {
      size_t current_index = matching_objects[i][j]; // 代表 point i对应的第j个目标的索引
                           ...
                           autoware_msgs::DetectedObjectArray& detected_objects_output)
{
  ...
  for (size_t i = 0; i < targets_.size(); i++)
  {
    ...
    dd.id = targets_[i].ukf_id_; // 目标id
    dd.velocity.linear.x = tv;   // 目标速度
    dd.acceleration.linear.y = tyaw_rate;// 目标角加速度
    dd.velocity_reliable = targets_[i].is_stable_; // 初始化默认false
    dd.pose_reliable = targets_[i].is_stable_;     // 初始化默认false

    // is_static_ 初始化默认false
    if (!targets_[i].is_static_ && targets_[i].is_stable_)
    {
      ...
      dd.pose.position.x = tx; // 目标 x
      dd.pose.position.y = ty; // 目标 y
      // 不是nan点，给box框传递方向
      if (!std::isnan(q[0]))
