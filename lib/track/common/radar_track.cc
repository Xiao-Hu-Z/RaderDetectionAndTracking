...
RadarTrack::RadarTrack(const ObjectPtr& obs, const double timestamp) {
  s_current_idx_ %= MAX_RADAR_IDX;
  obs_id_ = s_current_idx_++;
  obs_radar_ = ObjectPool::Instance().Get();
  *obs_radar_ = *obs;
  obs_ = ObjectPool::Instance().Get();
  *obs_ = *obs;
...
void RadarTrack::UpdataObsRadar(const ObjectPtr& obs_radar,
...
ObjectPtr RadarTrack::GetObs() { return obs_; }
